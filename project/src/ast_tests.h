...
public:
    ...
    /*! \brief Ensures the root of the function can be parsed

        Tests that Root and EmptyStmts are correctly integrated
        into the parser

        This was the first test to pass on 11/18/2014
    */
    void test_my_test_1 () { unparse_tests("my_sample_1.dsl"); }

    /*! \brief Ensures variables can be declared

        Tests that StandardDecl, MatrixDecl, and MatrixAdvancedDecl 
        are all correctly integrated into the parser

        This was the second test to pass on 11/18/2014
    */
    void test_my_test_2 () { unparse_tests("my_sample_2.dsl"); }

    /*! \brief Ensures if statements can be parsed and variables can be assigned

        Tests that IfElseStmt, StandardAssignStmt, and MatrixAssignStmt
        are correctly integrated into the parser

        This was the third test to pass on 11/19/2014
    */
    void test_my_test_3 () { unparse_tests("my_sample_3.dsl"); }

    /*! \brief Ensures loops can be parsed and expressions can be printed

        Tests that WhileStmt, ForStmt, and PrintStmt can be parsed.

        This was the fourth test to pass on 11/19/2014
    */
    void test_my_test_4 () { unparse_tests("my_sample_4.dsl"); }

    /*! \brief Ensures everything can be parsed

        Tests all classes declared in AST.h

        This was the fifth test to pass on 11/20/2014
    */
    void test_my_test_5 () { unparse_tests("my_sample_5.dsl"); }
